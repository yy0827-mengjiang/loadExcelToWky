...
public class ConnectionPool4MysqlDruid {

	private static Logger sLogger = Logger
			.getLogger(ConnectionPool4MysqlDruid.class);

	private static ConnectionPool4MysqlDruid pool;
	 ...
	 * Ctor.
	 */
	private ConnectionPool4MysqlDruid() {
	}
...
//			}
	 ...
	 * @return
	 */
	public static final DruidPooledConnection getConn() {
	 ...
	 * @return
	 */
	public static synchronized final ConnectionPool4MysqlDruid getInstance() {
		if (pool == null) {
			try {
				pool = new ConnectionPool4MysqlDruid();
			} catch (Exception e) {
	 ...
	 * @param con
	 */
	public void free(ResultSet rs, PreparedStatement stmt, DruidPooledConnection con) {
		try {
			if (rs != null)
				rs.close();
			if (stmt != null)
