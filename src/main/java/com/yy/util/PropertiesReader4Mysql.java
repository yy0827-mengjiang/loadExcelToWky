 ...
 * 专为加载配置文件所用
 * */
public class PropertiesReader4Mysql {
	private static Logger msgLogInfo = Logger.getLogger("msgInfo");
	private static String PROPFILE = "config.mysql.properties";
	...
	private static void loadFile() {
		...
		if (configPath == null) {
			msgLogInfo.error("无法加载配置文件config.mysql.propertie路径，请检查该配置文件路径是否正确！");
		} else {
			try {
				...
				msgLogInfo.error("加载配置文件config.mysql.propertie出现异常!");
				e.printStackTrace();
			}
		}
	}
